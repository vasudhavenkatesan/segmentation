...
def predict(net, image, input_dim, device):
    ...
    for i in range(0, n_preds):
        image = torch.from_numpy(img.__getitem__(i)[0]).to(device=device, dtype=torch.float32)
        mask = torch.from_numpy(img.__getitem__(i)[1]).to(device=device, dtype=torch.float32)
        plt.subplot(1, 3, 1)
        plt.title(f'Image')
        ...
        with torch.no_grad():
            ...
            plt.imshow(pred_for_plot[10, :, :], cmap='gray')
